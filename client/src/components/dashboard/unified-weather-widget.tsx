...
const DayForecast: React.FC<DayForecastProps> = ({ day, temperature, weatherCode, isActive = false, isToday = false }) => {
  return (
    <div className={`flex flex-col items-center py-2 px-2 sm:px-3 min-w-[60px] sm:min-w-[80px] flex-shrink-0 transition-all duration-200 hover:bg-white/5 rounded-lg ${isActive ? 'bg-white/10' : ''}`}>
      <div className="text-xs font-medium text-white/90 mb-1 truncate">
        {isToday ? 'Today' : day.slice(0, 3)}
      </div>
      <WeatherIcon weatherCode={weatherCode} className="h-4 w-4 sm:h-5 sm:w-5 mb-1 text-white/90" />
      <div className="text-xs sm:text-sm font-medium text-white">{temperature}°</div>
    </div>
  );
};
...
const ForecastSection: React.FC<ForecastSectionProps> = ({ dailyForecast, isMobile }) => {
  ...
  return (
    <div className="border-t border-white/10 pt-4 sm:pt-6">
      <div className="text-base sm:text-lg font-medium text-white/90 mb-3 sm:mb-4">5-Day Forecast</div>
      <div className="relative">
        {/* Left hover zone - only on desktop */}
        ...
        {/* Right hover zone - only on desktop */}
        ...
        <div
          ...
          className="flex overflow-x-auto sm:overflow-x-hidden gap-1 sm:gap-2 scroll-smooth [&::-webkit-scrollbar]:hidden"
        ...
        >
          {dailyForecast?.slice(0, 5).map((day, index) => (
            ...
            <div key={i} className="flex flex-col items-center px-2 sm:px-3 min-w-[60px] sm:min-w-[80px] flex-shrink-0 opacity-50">
              <div className="text-xs text-white/50 mb-1">--</div>
              <Cloud className="h-4 w-4 sm:h-5 sm:w-5 mb-1 text-white/50" />
              <div className="text-xs sm:text-sm text-white/50">--°</div>
            </div>
          ))}
        </div>
      </div>
...
export const UnifiedWeatherWidget: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="bg-black/30 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/10 w-full max-w-md">
        <div className="text-white w-full">
          {/* Main weather info skeleton */}
          <div className="animate-pulse mb-6 sm:mb-8">
            <div className="h-16 sm:h-24 w-32 sm:w-40 bg-white/10 rounded-md mb-4"></div>
            <div className="h-5 sm:h-6 w-40 sm:w-48 bg-white/10 rounded-md mb-4"></div>
            <div className="grid grid-cols-3 gap-2 sm:gap-4">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="text-center">
                  <div className="h-3 w-8 sm:w-12 bg-white/10 rounded-md mb-1 mx-auto"></div>
                  <div className="h-3 sm:h-4 w-6 sm:w-8 bg-white/10 rounded-md mx-auto"></div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Forecast skeleton */}
          <div className="border-t border-white/10 pt-4 sm:pt-6">
            <div className="h-4 sm:h-5 w-24 sm:w-32 bg-white/10 rounded-md mb-3 sm:mb-4"></div>
            <div className="flex gap-1 sm:gap-2">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="flex flex-col items-center px-2 sm:px-3 min-w-[60px] sm:min-w-[80px] animate-pulse">
                  <div className="h-3 w-6 sm:w-8 bg-white/10 rounded-md mb-1"></div>
                  <div className="h-4 w-4 sm:h-5 sm:w-5 bg-white/10 rounded-full mb-1"></div>
                  <div className="h-3 w-4 sm:w-6 bg-white/10 rounded-md"></div>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="bg-black/30 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/10 shadow-2xl w-full max-w-md">
      <div className="text-white w-full">
        {/* Main Weather Information */}
        <div className="mb-6 sm:mb-8">
          {/* Large temperature display */}
          <div className="text-6xl sm:text-[80px] lg:text-[100px] font-light leading-none mb-2">
            {currentWeather?.temperature}°
          </div>
          
          {/* Weather description */}
          <div className="text-lg sm:text-xl lg:text-2xl font-light mb-4 text-white/90">
            {currentWeather ? getWeatherDescription(currentWeather.weatherCode) : 'Loading...'}
          </div>
          
          {/* Additional info row - more compact */}
          <div className="grid grid-cols-3 gap-2 sm:gap-4 text-white/80">
            ...
            <div className="text-center">
              ...
              <div className="text-xs sm:text-sm font-medium">{currentWeather?.humidity || "67"}%</div>
            </div>
            <div className="text-center">
              <div className="text-xs opacity-70 mb-1">Wind</div>
              <div className="flex items-center justify-center gap-1">
                <Navigation 
                  className="h-2.5 w-2.5 sm:h-3 sm:w-3 opacity-70" 
                  style={{ 
                    transform: `rotate(${(currentWeather?.windDirection || 0) + 45}deg)` 
                  }} 
                />
                <div className="text-xs sm:text-sm font-medium">{currentWeather?.windSpeed || "12"}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* 5-Day Forecast */}
        <ForecastSection dailyForecast={dailyForecast} isMobile={isMobile} />
      </div>
    </div>
  );
};